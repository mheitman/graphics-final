...
void ObjParser::load_obj(const char* filename,
    ...
    while (getline(in, line))
    {
        if (line.substr(0,2) == "v ") {
            ...
            glm::vec3 &v0 = non_adjusted_vertices[pair[0].vertex];
            glm::vec3 &v1 = non_adjusted_vertices[pair[0].vertex];
            glm::vec3 &v2 = non_adjusted_vertices[pair[2].vertex];

            // Shortcuts for UVs
            glm::vec2 &uv0 = non_adjusted_uvs[pair[0].uv];
            glm::vec2 &uv1 = non_adjusted_uvs[pair[1].uv];
            glm::vec2 &uv2 = non_adjusted_uvs[pair[2].uv];
